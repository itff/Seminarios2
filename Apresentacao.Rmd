...
## Interpretações dos efeitos
  ...
  * \(\beta = 0.5\): risco aumenta em 0.5 para cada unidade aumentada em \(X_{ij}\);

  * \(\alpha < 0\): risco de quem recebe tratamento \(Z_i = 1\) é menor de quem não recebe \(Z_i = 0\);

  * \(\gamma = 0\): o tratamento não influencia os valores dos dados longitudinais.

<!--
Portanto, falaremos a interpretação que de cada efeito traz para o modelo.
O \(\beta = 0\) ou 0 estar presente no intervalo de confiança da estimativa de Beta, significa que os dados longitudinais não tem associação com o tempo até o evento. Quer dizer que os dados longitudinais não alteram a taxa de falha, portanto, não precisa-se de modelo conjunto neste caso. 
...
Se \(\gamma = 0\) significa que que o tratamento não influência os valores dos dados longitudinais.

Agora passaremos para o Ruy falar sobre os modelos que utilizamos para comparar com o modelo conjunto.
-->
...
* Caracterização da associação entre tempo até o evento e QDV atravéz modelagem conjunta;
