...
<!-- Esse trabalho foi feito por mim, Ítalo, e pelo Ruy, e fomos orientados pela Juliana.
Apresentaremos o artigo 'Basic Concepts and Methods for Joint Models of Longitudinal and Survival Data' de Ibrahim, chu e chen.
-->
...
<!--
introduçao, primeiro falares o que é um dado de tempo até o evento.
Nesse dado acompanhamos um paciente no tempo até a ocorencia do evento que poder como por exemplo a morte ou a censura à direita.

A censura é quando o paciente sai do estudo ou o estudo termina e não temos todas informações até o evento, porque não evento ainda não ocerreu. 

Esse tempo até o evento é primordial na análise de sobrevivencia, porque esses dados no modelo nos permite, comparar o tempo até o evendo de um paciente com tratamento do paciente sem tratamento ou com um tratamento diferente. O tratamento pode ser um remédio por exemplo.

Portanto, o modelo consegue nos retornar a taxa de falha em cada grupo e com ela conseguimos calcular o risco relativo que compara os grupos falando qual grupo tem uma maior chance do evento ocorrer em determinado tempo. E com isso conseguimos saber se o grupo com tratamento sobrevive mais do que o sem tratamento. -->
...
O importante para os modelos que utilizam esses dados é o valor observado em determinado tempo e em ensaios clínicos, esses valores observados podem influenciar o dado de tempo até o evento.

Um exemplo, é o dado de qualidade de vida de um paciente coletado toda semana, os valores desses dados podem estar relacionados com o dado de tempo-até-o-evento.
-->
...
O objetivo do artigo é demonstrar que, se existir uma influência do dado longitudinal no tempo-até-o-evento, essa modelagem conjunta produzira estimativas mais eficientes para o efeito do tratamento no tempo-até-o-evento e para o efeito do tratamento dos marcadores longitudinais. Além de reduzir o viés da estimativa do efeito geral do tratamento.

Para demonstrar essas propriedades, mais a frente, demonstramos um exemplo e uma simulações para comparar os modelos.
-->
...
* $Y_{ij} = X_{ij} +\epsilon_{ij}$;
  ...
  * $Y_{ij}$ é a resposta observada do i-ésimo indivíduo no j-ésimo tempo;
...
<!--
Para fazer o modelo conjunto, adicionamos a função trajetória do componente longitudinal a função de riscos do componente de sobrevivência. Portanto, o modelo conjunto é escrito desse forma: ...
-->
...
<!--
Portanto, falaremos a interpretação que de cada efeito traz para o modelo nesses exemplos:
O \(\beta = 0\) significa que os dados longitudinais não tem associação com o tempo
...
Dando um exemplo com \(\beta = 0.5\), significa que o risco aumenta em 0.5 para cada unidade 
aumentada na função trajetória (X_ij) que qualquer grupo de tratamento. Se for negativo esse risco diminui.
